...
from .forms import TaskCreateForm,TaskForm,SignupForm
...
def signin(request):
  if request.method == "POST":
    ...
    user = authenticate(request, username=username, password=password)####откуда импортировать?
    if user is not None:
      login(request, user)  # Вход пользователя####откуда импортировать?
...
def signout(request):
  # Выходим из системы
  logout(request)####откуда импортировать?
